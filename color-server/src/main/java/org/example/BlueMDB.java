...
@MessageDriven(activationConfig = {
        ...
        @ActivationConfigProperty(propertyName = "messageSelector", propertyValue = "color = 'BLUE'")
})



public class BlueMDB implements MessageListener {
    ...
    public void onMessage(Message message) {
        try {
            if (message instanceof MapMessage) {
                count++;
                System.out.println("BLUE received, count: " + count);
                if (count % 10 == 0) {
                    sendStatistics("java:/queue/colorStatistics", "BLUE");
                }
            }
        } catch (Exception e) {
            e.printStackTrace();
